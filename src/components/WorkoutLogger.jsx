...
export default function WorkoutLogger({ user, exercises, workouts, addWorkout, exercisesById }) {
  const [date, setDate] = useState(todayISO());
  const [name, setName] = useState("Workout");

  const [query, setQuery] = useState("");
  const [entries, setEntries] = useState([]); // {exerciseId, sets:[{reps, weight, rpe}]}
  ...
  const pickExercise = (id) => {
    const exists = entries.some((e) => e.exerciseId === id);
    if (!exists) {
      setEntries((prev) => [...prev, { exerciseId: id, sets: [{ reps: 8, weight: 0, rpe: 0 }] }]);
    }
  };
                                  ...
                                  <TableRow key={idx}>
                                    ...
                                    <TableCell>
                                      <Input
                                        ...
                                        type="number"
                                        value={s.reps}
                                        onChange={(ev) => updateSet(e.exerciseId, idx, { reps: safeNum(ev.target.value) })}
                                      />
                                    </TableCell>
                                    <TableCell>
                                      <Input
                                        ...
                                        inputMode="decimal"
                                        value={s.weight}
                                        onChange={(ev) => updateSet(e.exerciseId, idx, { weight: safeNum(ev.target.value) })}
                                      />
                                    </TableCell>
                                    {user.settings.showRPE ? (
                                      <TableCell>
                                        <Input
                                          ...
                                          inputMode="decimal"
                                          value={s.rpe || 0}
                                          onChange={(ev) => updateSet(e.exerciseId, idx, { rpe: safeNum(ev.target.value) })}
                                        />
                                      </TableCell>
                                    ) : null}
                                    <TableCell>
